...
COPY --from=powerman/dockerize:0.20.2 /usr/local/bin/dockerize /usr/local/bin
...
RUN set -ex && \
    ...
    mkdir /tmp/version && \
    curl -sL "http://dl-cdn.alpinelinux.org/alpine/v3.22/community/x86_64/APKINDEX.tar.gz" | tar -xz -C /tmp/version && \
